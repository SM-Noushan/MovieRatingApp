...
const Add = () => {
    ...
    const formik = useFormik({
        initialValues: {
          ...
          validationSchema: yup.object({
            ...
            status: yup.string().oneOf(['Active', 'Banned']).required('Required').min(6).max(6),
            userid: yup.string().email('Must be a Valid Email Address').required('Required'),
            password: yup.string().min(6, 'Minimum Password Length 6').max(32),
            }),
          onSubmit: (values) => {
            // console.log(values);
            axiosConfig.post(`http://127.0.0.1:8000/api/admin/user/edit/${location.state.id}`, values)
            .then(response=>{
    ...
    const renderStatusError = formik.touched.status && formik.errors.status && (
  ...
  return (
        ...
        <Card.Header>
            <Breadcrumb>
                ...
                <Breadcrumb.Item linkAs={Link} linkProps={{ to: "/admin/user/view" }}> Users </Breadcrumb.Item>
                <Breadcrumb.Item active>Edit</Breadcrumb.Item>
            </Breadcrumb>
        </Card.Header>
        <Card.Body>
            <Card.Title className="mb-4">Update User's information</Card.Title>
            <div>
                ...
                { show ? <Alert variant="danger" onClose={() => setShow(false)} dismissible>
                                ...
                                {servererrors['errors']['status'] && servererrors['errors']['status'].map((err, i)=><p className="pb-0 mb-0" key={i}>{err}</p>)}
                            </Alert> : null}
                ...
                <Form.Group as={Row} className="mb-3">
                    ...
                    <Col sm={10}>
                    <Form.Control type="text" name="userid" value={formik.values.userid} onChange={formik.handleChange} placeholder="Email" required />
                    {renderUseridError}
                    </Col>
                </Form.Group>
                ...
                <Form.Group as={Row} className="mb-3">
                    ...
                    <Col sm={10}>
                    <Form.Control type="text" name="status" value={formik.values.status} onChange={formik.handleChange} placeholder="Name" required />
                    {renderStatusError}
                    </Col>
                </Form.Group>
